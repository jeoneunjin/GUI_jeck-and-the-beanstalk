...
music1 = os.path.join(current_dir, 'assets', 'backmusic.mp3')
startback = os.path.join(current_dir, 'assets', 'startback.png')
rule = os.path.join(current_dir, 'assets', 'stage1rule.png')
game  = os.path.join(current_dir, 'assets', 'game.jpg')
start_image = os.path.join(current_dir, 'assets', 'start_image.jpg')
music3 = os.path.join(current_dir, 'assets', 'Applause.wav')
# 색 설정
white=(255,255,255)
black=(0,0,0)
green= (60, 179, 113)
...
# 배경 음악
music = pg.mixer.music.load(music1)
pg.mixer.music.play(-1)
# 배경 이미지 불러오기
startback = pg.image.load(startback).convert()
stage1_image = pg.image.load(rule).convert()
ingame = pg.image.load(game).convert()
start_image = pg.image.load(start_image).convert()
# 초당 프레임 설정
clock = pg.time.Clock()
# 텍스트 함수
...
# 잭 캐릭터 이미지 
jeckr3 = os.path.join(current_dir, 'assets', 'jeckr3.png')
jeckr2 = os.path.join(current_dir, 'assets', 'jeckr2.png')
jeckr1 = os.path.join(current_dir, 'assets', 'jeckr1.png')
jeckl3 = os.path.join(current_dir, 'assets', 'jeckl3.png')
jeckl2 = os.path.join(current_dir, 'assets', 'jeckl2.png')
jeckl1 = os.path.join(current_dir, 'assets', 'jeckl1.png')
jeck_image = [pg.image.load(jeckr3),pg.image.load(jeckr2),pg.image.load(jeckr1),pg.image.load(jeckl1),pg.image.load(jeckl2),pg.image.load(jeckl3)]
# 새와 장애물 이미지 
bird = os.path.join(current_dir, 'assets', 'bird.png')
ob = os.path.join(current_dir, 'assets', 'obstacle.png')
bird_image = pg.image.load(bird).convert_alpha()
ob_image = pg.image.load(ob).convert_alpha()
# 잭 캐릭터 움직임 
move = False
walkCount = 0
...
def gameLoop() :
  ...
  stage1rule()
  # 배경음악2
  music2 = os.path.join(current_dir, 'assets', 'music.mp3')
  music = pg.mixer.music.load(music2)
  pg.mixer.music.play(-1) 
  # 초기 잭의 위치 
  lead_x=300
  lead_y=300
  ...
  while stage1_start :
    ...
    if displaytime < 0:
      ...
      while c :
        ...
        music = pg.mixer.music.load(music3)
        pg.mixer.music.play()     
